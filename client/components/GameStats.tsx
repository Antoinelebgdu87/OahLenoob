...
export function GameStats({ wins, losses, totalEarnings, lastResult, className }: GameStatsProps) {
  ...
  return (
    <Card className={cn("p-6 bg-card/95 backdrop-blur border-border/50", className)}>
      <h3 className="text-lg font-semibold text-foreground mb-4 text-center">Game Statistics</h3>
      ...
      <div className="text-center mb-4">
        <div className="text-lg font-semibold text-foreground">Win Rate: {winRate}%</div>
        <div className={cn(
          "text-xl font-bold",
          totalEarnings >= 0 ? "text-green-400" : "text-red-400"
        )}>
          Total: {totalEarnings >= 0 ? '+' : ''}{totalEarnings}€
        </div>
      </div>
      ...
      {lastResult && (
        <div className="border-t border-border pt-4">
          <div className="text-center">
            <div className="text-sm text-muted-foreground mb-2">Last Result</div>
